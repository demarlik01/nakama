...
**현재 문제:**
- 에이전트 워크스페이스에 memory/ 구조는 있지만 아무것도 기록 안 함
- 시스템 프롬프트에 메모리 작성 지침 부족
- heartbeat 미설정 → 주기적 메모리 정리/요약 안 됨

**구현 방안:**
- AGENTS.md에 메모리 작성 지침 추가 (대화 중 중요한 결정/선호도/컨텍스트 기록)
- heartbeat 크론 설정 → 주기적으로 memory 정리, MEMORY.md 업데이트
- 세션 시작 시 최근 memory 파일 자동 로드 (시스템 프롬프트 주입)
