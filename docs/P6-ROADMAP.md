...
**목표:** 에이전트가 생성한 파일을 슬랙 응답에 자동으로 첨부
...
**구현 방안 (응답 파이프라인 자동 첨부):**
- LLM 턴 실행 전: 워크스페이스 파일 스냅샷 (또는 mtime 기록)
- LLM 턴 실행 후: 새로 생성/수정된 파일 감지
- 슬랙 응답 보낼 때 신규 파일을 `files.uploadV2`로 자동 첨부
- 대상: 텍스트/코드/이미지 등 (바이너리 제외 or 사이즈 제한)
- 별도 툴 불필요 — LLM이 write만 하면 자동으로 붙음

**체크리스트:**
- [ ] 봇 토큰 `files:write` 스코프 확인
- [ ] 턴 전후 워크스페이스 파일 diff 로직
- [ ] 신규/수정 파일 → `files.uploadV2` 자동 업로드
- [ ] 사이즈/타입 필터 (너무 큰 파일, node_modules 등 제외)
- [ ] 현재 대화 채널/스레드에 자동 타겟팅
- [ ] 테스트: 에이전트가 파일 생성 → 슬랙 응답에 자동 첨부

## Phase 7: 이미지 비전 (멀티모달 입력)
...
**구현 방안:**
- 이미지 파일 다운로드 후 base64 인코딩
- Pi SDK 메시지에 이미지 content block으로 포함:
  ```ts
  { type: "image", source: { type: "base64", media_type: "image/jpeg", data: "..." } }
  ```
- 지원 포맷: jpeg, png, gif, webp
- 비이미지 파일은 기존 방식 유지 (경로 텍스트)
- 다운로드 인증 검증: `url_private` + Bearer 토큰이 정상 작동하는지 재확인

**체크리스트:**
- [ ] 이미지 다운로드 → base64 변환 유틸
