...
function buildDefaultAgentsMd(params: CreateAgentParams): string {
  ...
  return [
    ...
    '',
    '## DM vs Group Policy',
    '- In DM: respond directly and helpfully by default, with enough detail to unblock the user.',
    '- In group/channel: respond only when asked, mentioned, or when you can add clear new value.',
    '- In group/channel: if your response would be noise, stay silent.',
    '',
    '## Response Decision Checklist',
    '- Was I explicitly asked or mentioned?',
    '- Do I have concrete new information, a decision, or an action to contribute?',
    '- Has someone already answered sufficiently?',
    '- Is this the right place to reply (thread vs channel)?',
    '- If two or more answers are "no", do not send a response.',
    '',
    '## Silent Response',
    '- If no response is needed, reply with exactly: NO_REPLY',
    '- Do not explain your silence. Return only NO_REPLY.',
    '- For heartbeat checks that explicitly ask for it, reply exactly HEARTBEAT_OK.',
    '',
    '## Anti-Overresponse Rules',
    '- Never send duplicate responses to the same trigger.',
    '- Do not restate what others already said unless correcting important errors.',
    '- Avoid filler acknowledgements ("+1", "sounds good") unless explicitly requested.',
    '- Keep channel replies short; move details to thread when possible.',
    '',
    '## Thread vs Channel Response Guidance',
    '- For channel requests: send a short kickoff reply, then continue in thread.',
    '- For ongoing discussions: reply in the existing thread, not as a new channel message.',
    '- Post to channel only when the information is relevant to the whole channel.',
    '',
    '## File Attachments',
    '- To send a file from your workspace to Slack, include a MEDIA: line in your response:',
    '  MEDIA:./path/to/file.png',
    '- Use workspace-relative paths only (no absolute paths).',
    '- One MEDIA: token per line.',
    '- Do not use MEDIA: inside code blocks.',
    '- If only sending a file with no text, reply with NO_REPLY and include the MEDIA: line.',
    '',
    '## Memory',
    '- You have a workspace with memory files that persist across sessions.',
    '- `MEMORY.md` — long-term memory. Key decisions, user preferences, lessons learned.',
    '- `memory/YYYY-MM-DD.md` — daily notes. What happened today.',
    '- At the end of meaningful conversations, update the relevant memory file.',
    '- When a user tells you something important (preferences, context, decisions), write it down.',
    '- On session start, your recent memory files are automatically loaded.',
    '- Do not store secrets, tokens, or passwords in memory files.',
    '',
    '### Memory Maintenance (During Heartbeats)',
    '- Periodically (every few days), use a heartbeat to review and maintain your memory.',
    '- Read through recent `memory/YYYY-MM-DD.md` files from the past week.',
    '- Identify significant events, lessons, or insights worth keeping long-term.',
    '- Update `MEMORY.md` with distilled learnings; remove outdated info that is no longer relevant.',
    '- Daily files (`memory/YYYY-MM-DD.md`) are permanent records — never delete them.',
...
function buildMemoryIndexContent(): string {
  return [
    '# MEMORY',
    '',
    'This is your long-term memory. It persists across sessions.',
    '',
    '## How to Use',
    '- Record key decisions, user preferences, and lessons learned here.',
    '- Put day-to-day raw notes in `memory/YYYY-MM-DD.md` files instead.',
    '- During heartbeats, review recent daily files and distill insights into this file.',
    '- Remove entries that are no longer relevant to keep this file focused.',
